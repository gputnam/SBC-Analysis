...
if __name__ == "__main__":
    ...
    dat = dict(acoustic=rb("/exp/e961/data/users/gputnam/test-daq/knock.sbc.bin"))
    dat["acoustic"]["loaded"] = True

    nsample = 0
    PERIOD = 10 # ns
    for k in list(dat["acoustic"].keys()):
        m = re.search("Channel(\d+)", k)
        if m:
            rnge = dat["acoustic"][k][0] / 1e3 # mV -> V
            offset = dat["acoustic"][k][1] / 1e3
            dat["acoustic"][k] = dat["acoustic"][k][2:]*rnge + offset
            nsample = dat["acoustic"][k].size
    dat["acoustic"]["time"] = np.arange(nsample)*PERIOD
            
    out.append(AcousticAnalysis(dat, tau))
